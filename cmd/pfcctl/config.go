...
const (
	...
	unusableFlags = pfcjson.UFWebsocketOnly | pfcjson.UFNotification
)
...
func listCommands() {
	...
	cmdMethods := pfcjson.RegisteredCmdMethods()
	categorized := make([][]string, numCategories)
	for _, method := range cmdMethods {
		flags, err := pfcjson.MethodUsageFlags(method)
		if err != nil {
		...
		if flags&pfcjson.UFWalletOnly != 0 {
