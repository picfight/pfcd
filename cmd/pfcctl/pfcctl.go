...
func commandUsage(method string) {
	usage, err := pfcjson.MethodUsageText(method)
	if err != nil {
...
func main() {
	...
	method := args[0]
	usageFlags, err := pfcjson.MethodUsageFlags(method)
	if err != nil {
	...
	cmd, err := pfcjson.NewCmd(method, params...)
	if err != nil {
		// Show the error along with its error code when it's a
		// pfcjson.Error as it reallistcally will always be since the
		...
		// The error is not a pfcjson.Error and this really should not
