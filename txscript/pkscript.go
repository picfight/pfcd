...
const (
	...
	//   Public key (33 byte)
	minPubKeyHashSigScriptLen = 1 + pfcec.MinSigLen + 1 + 1 + 33
...
func (s PkScript) Address(chainParams *chaincfg.Params) (pfcutil.Address, error) {
...
func computeNonWitnessPkScript(sigScript []byte) (PkScript, error) {
	...
	case len(sigScript) >= minPubKeyHashSigScriptLen &&
		...
		if pfcec.IsCompressedPubKey(pubKey) {
