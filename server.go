...
import (
	...
	"github.com/picfight/pfcutil/bloom"
)
...
var (
	...
	// ourselves to other picfightcoin peers.
	userAgentName = "pfcd"

	// userAgentVersion is the user agent version and is used to help
	// identify ourselves to other picfightcoin peers.
...
// broadcastMsg provides the ability to house a picfightcoin message to be broadcast
...
// server provides a picfightcoin server for handling communications to and from
// picfightcoin peers.
type server struct {
...
// OnVersion is invoked when a peer receives a version picfightcoin message
...
// OnMemPool is invoked when a peer receives a mempool picfightcoin message.
...
// OnTx is invoked when a peer receives a tx picfightcoin message.  It blocks
// until the picfightcoin transaction has been fully processed.  Unlock the block
...
// OnBlock is invoked when a peer receives a block picfightcoin message.  It
// blocks until the picfightcoin block has been fully processed.
func (sp *serverPeer) OnBlock(_ *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	...
	// until the picfightcoin block is fully processed and known
	...
	// the picfightcoin block has been fully processed.
	sp.server.syncManager.QueueBlock(block, sp.Peer, sp.blockProcessed)
	<-sp.blockProcessed
}

// OnInv is invoked when a peer receives an inv picfightcoin message and is
...
// handleGetData is invoked when a peer receives a getdata picfightcoin message and
...
// OnGetCFilters is invoked when a peer receives a getcfilters picfightcoin message.
...
// OnGetCFHeaders is invoked when a peer receives a getcfheader picfightcoin message.
...
// OnGetCFCheckpt is invoked when a peer receives a getcfcheckpt picfightcoin message.
...
// OnFeeFilter is invoked when a peer receives a feefilter picfightcoin message and
// is used by remote peers to request that no transactions which have a fee rate
// lower than provided value are inventoried to them.  The peer will be
// disconnected if an invalid fee filter value is provided.
func (sp *serverPeer) OnFeeFilter(_ *peer.Peer, msg *wire.MsgFeeFilter) {
	// Check that the passed minimum fee is a valid amount.
	if msg.MinFee < 0 || msg.MinFee > pfcutil.MaxSatoshi {
		peerLog.Debugf("Peer %v sent an invalid feefilter '%v' -- "+
			"disconnecting", sp, pfcutil.Amount(msg.MinFee))
		sp.Disconnect()
		return
	}
...
// OnGetAddr is invoked when a peer receives a getaddr picfightcoin message
...
// OnAddr is invoked when a peer receives an addr picfightcoin message and is
...
func (s *server) TransactionConfirmed(tx *pfcutil.Tx) {
	...
	if !cfg.DisableDNSSeed {
		...
		connmgr.SeedFromDNS(activeNetParams.Params, defaultRequiredServices,
			btcdLookup, func(addrs []*wire.NetAddress) {
				// Picfightcoind uses a lookup of the dns seeder here. This
...
// picfightcoin network type specified by chainParams.  Use start to begin accepting
...
func newServer(listenAddrs, agentBlacklist, agentWhitelist []string,
	...
	txC := mempool.Config{
		...
		CalcSequenceLock: func(tx *pfcutil.Tx, view *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error) {
