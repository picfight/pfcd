...
// not needed.
func internalRPCError(errStr, context string) *pfcjson.RPCError {
	logStr := errStr
	if context != "" {
...
func messageToHex(msg wire.Message) (string, error) {
	...
	if err := msg.PfcEncode(&buf, maxProtocolVersion, wire.WitnessEncoding); err != nil {
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	for encodedAddr, amount := range c.Amounts {
		...
		if amount <= 0 || amount > pfcutil.MaxSatoshi {
			return nil, &pfcjson.RPCError{
		...
		addr, err := pfcutil.DecodeAddress(encodedAddr, params)
		if err != nil {
			return nil, &pfcjson.RPCError{
		...
		satoshi, err := pfcutil.NewAmount(amount)
		if err != nil {
...
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []pfcjson.Vout {
	...
	for i, v := range mtx.TxOut {
		...
		vout.N = uint32(i)
		vout.Value = pfcutil.Amount(v.Value).ToPFC()
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx,
	...
	blkHeight int32, chainHeight int32) (*pfcjson.TxRawResult, error) {
	...
	txReply := &pfcjson.TxRawResult{
		...
		Vsize:    int32(mempool.GetTxVirtualSize(pfcutil.NewTx(mtx))),
...
func handleDecodeRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if err != nil {
		return nil, &pfcjson.RPCError{
			Code:    pfcjson.ErrRPCDeserialization,
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	p2sh, err := pfcutil.NewAddressScriptHash(script, s.cfg.ChainParams)
	if err != nil {
...
func handleGetAddedNodeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if c.Node != nil {
		...
		if !found {
			return nil, &pfcjson.RPCError{
				Code:    pfcjson.ErrRPCClientNodeNotAdded,
	...
	for _, rpcPeer := range peers {
		...
		addrs := make([]pfcjson.GetAddedNodeInfoResultAddr, 0, len(ipList))
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	blk, err := pfcutil.NewBlockFromBytes(blkBytes)
	if err != nil {
...
func handleGetBlockChainInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	chainInfo := &pfcjson.GetBlockChainInfoResult{
		Chain:         params.Name,
		...
		Pruned:        false,
		Bip9SoftForks: make(map[string]*pfcjson.Bip9SoftForkDescription),
	}
	...
	chainInfo.SoftForks = []*pfcjson.SoftForkDescription{
		{
			ID:      "bip34",
			Version: 2,
			Reject: struct {
	...
	for deployment, deploymentDetails := range params.Deployments {
		...
		chainInfo.Bip9SoftForks[forkName] = &pfcjson.Bip9SoftForkDescription{
...
// state as a pfcjson.GetBlockTemplateResult that is ready to be encoded to JSON
// and returned to the caller.
//
// This function MUST be called with the state locked.
func (state *gbtWorkState) blockTemplateResult(useCoinbaseValue bool, submitOld *bool) (*pfcjson.GetBlockTemplateResult, error) {
...
func handleGetBlockTemplateRequest(s *rpcServer, request *pfcjson.TemplateRequest, closeChan <-chan struct{}) (interface{}, error) {
	...
	if !(cfg.RegressionTest || cfg.SimNet) &&
		...
		return nil, &pfcjson.RPCError{
			...
			Message: "Picfightcoin is not connected",
		}
	}
	...
	if currentHeight != 0 && !s.cfg.SyncMgr.IsCurrent() {
		return nil, &pfcjson.RPCError{
			...
			Message: "Picfightcoin is downloading blocks...",
		}
	}
...
func handleGetInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &pfcjson.InfoChainResult{
		...
		RelayFee:        cfg.minRelayTxFee.ToPFC(),
	}
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	txOutReply := &pfcjson.GetTxOutResult{
		...
		Value:         pfcutil.Amount(value).ToPFC(),
	...
	for _, txIn := range mtx.TxIn {
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &pfcjson.PrevOut{
				...
				Value:     pfcutil.Amount(originTxOut.Value).ToPFC(),
			}
		}
	}
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	params := s.cfg.ChainParams
	addr, err := pfcutil.DecodeAddress(c.Address, params)
	if err != nil {
		return nil, &pfcjson.RPCError{
			Code:    pfcjson.ErrRPCInvalidAddressOrKey,
			Message: "Invalid address or key: " + err.Error(),
	...
	for i := range addressTxns {
		...
		if blkHeader != nil {
			// This is not a typo, they are identical in Picfightcoin
			// Core as well.
	...
	if err != nil {
		...
		if txRuleErr, ok := ruleErr.Err.(mempool.TxRuleError); ok {
			...
			case strings.Contains(
				...
				code = pfcjson.ErrRPCTxAlreadyInChain

			default:
				code = pfcjson.ErrRPCTxRejected
			}
		}
...
func handleValidateAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	addr, err := pfcutil.DecodeAddress(c.Address, s.cfg.ChainParams)
	if err != nil {
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	var buf bytes.Buffer
	wire.WriteVarString(&buf, 0, "Picfightcoin Signed Message:\n")
	wire.WriteVarString(&buf, 0, c.Message)
	expectedMessageHash := chainhash.DoubleHashB(buf.Bytes())
	pk, wasCompressed, err := pfcec.RecoverCompact(pfcec.S256(), sig,
		expectedMessageHash)
	if err != nil {
		// Mirror Picfightcoin Core behavior, which treats error in
	...
	address, err := pfcutil.NewAddressPubKey(serializedPK, params)
	if err != nil {
		// Again mirror Picfightcoin Core behavior, which treats error in public key
...
// standardCmdResult checks that a parsed command is a standard Picfightcoin JSON-RPC
...
func parseCmd(request *pfcjson.Request) *parsedRPCCmd {
	...
	if err != nil {
		...
		if jerr, ok := err.(pfcjson.Error); ok &&
			jerr.ErrorCode == pfcjson.ErrUnregisteredMethod {
		...
		parsedCmd.err = pfcjson.NewRPCError(
			pfcjson.ErrRPCInvalidParams.Code, err.Error())
		return &parsedCmd
	}
...
// the type *pfcjson.RPCError to the appropriate type as needed.
func createMarshalledReply(id, result interface{}, replyErr error) ([]byte, error) {
	...
	return pfcjson.MarshalResponse(id, result, jsonErr)
}
...
func (s *rpcServer) jsonRPCRead(w http.ResponseWriter, r *http.Request, isAdmin bool) {
	...
	// Terminate with newline to maintain compatibility with Picfightcoin Core.
...
type rpcserverSyncManager interface {
	...
	SubmitBlock(block *pfcutil.Block, flags blockchain.BehaviorFlags) (bool, error)
