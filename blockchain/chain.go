...
//
// This function is safe for concurrent access.
func (b *BlockChain) fetchMainChainBlockByHash(hash *chainhash.Hash) (*pfcutil.Block, error) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) fetchBlockByHash(hash *chainhash.Hash) (*pfcutil.Block, error) {
	// Check orphan cache.
	b.orphanLock.RLock()
...
func countSpentOutputs(block *pfcutil.Block, parent *pfcutil.Block) int {
...
// adding the block.
func countNumberOfTransactions(block, parent *pfcutil.Block) uint64 {
	var numTxns uint64
	if headerApprovesParent(&block.MsgBlock().Header) {
