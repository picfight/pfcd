...
func IsCoinBase(tx *pfcutil.Tx) bool {
...
func IsFinalizedTransaction(tx *pfcutil.Tx, blockHeight int32, blockTime time.Time) bool {
	msgTx := tx.MsgTx()
...
func CheckTransactionSanity(chainParams *chaincfg.Params, tx *pfcutil.Tx) error {
	...
	for _, txOut := range msgTx.TxOut {
		...
		// is detected and reported.  This is impossible for Picfightcoin, but
		...
		if totalSatoshi > pfcutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
// difficulty is in min/max range and that the block hash is less than the
// target difficulty as claimed.
func CheckProofOfWork(block *pfcutil.Block, powLimit *big.Int) error {
...
// txscript.
func CountSigOps(tx *pfcutil.Tx) int {
	msgTx := tx.MsgTx()
...
func CountP2SHSigOps(tx *pfcutil.Tx, isCoinBaseTx bool, utxoView *UtxoViewpoint) (int, error) {
...
func checkBlockSanity(chainParams *chaincfg.Params, block *pfcutil.Block, powLimit *big.Int, timeSource MedianTimeSource, flags BehaviorFlags) error {
	...
	// checks.  Picfightcoind builds the tree here and checks the merkle root
...
func CheckTransactionInputs(tx *pfcutil.Tx, txHeight int32, utxoView *UtxoViewpoint, chainParams *chaincfg.Params) (int64, error) {
	...
	for txInIndex, txIn := range tx.MsgTx().TxIn {
		...
		if originTxSatoshi < 0 {
			str := fmt.Sprintf("transaction output has negative "+
				"value of %v", pfcutil.Amount(originTxSatoshi))
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		if originTxSatoshi > pfcutil.MaxSatoshi {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v",
				pfcutil.Amount(originTxSatoshi),
				pfcutil.MaxSatoshi)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		...
		if totalSatoshiIn < lastSatoshiIn ||
			totalSatoshiIn > pfcutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkConnectBlock(node *blockNode, block *pfcutil.Block, view *UtxoViewpoint, stxos *[]SpentTxOut) error {
...
func (b *BlockChain) CheckConnectBlockTemplate(block *pfcutil.Block) error {
	b.chainLock.Lock()
