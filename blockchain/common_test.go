...
func loadBlocks(filename string) (blocks []*pfcutil.Block, err error) {
	...
	for height := int64(1); err == nil; height++ {
		...
		block, err = pfcutil.NewBlockFromBytes(rbytes)
		if err != nil {
			return
		}
...
func newFakeChain(params *chaincfg.Params) *BlockChain {
	...
	node := newBlockNode(params, &params.GenesisBlock.Header, nil)
...
func newFakeNode(chainParams *chaincfg.Params, parent *blockNode, blockVersion int32, bits uint32, timestamp time.Time) *blockNode {
	...
	return newBlockNode(chainParams, header, parent)
}
