...
func TestHaveBlock(t *testing.T) {
	...
	_, isOrphan, err := chain.ProcessBlock(pfcutil.NewBlock(&Block100000),
		BFNone)
	if err != nil {
...
func TestCalcSequenceLock(t *testing.T) {
	...
	utxoView.AddTxOuts(pfcutil.NewTx(unConfTx), 0x7fffffff)
	...
	tests := []struct {
		...
		{
			tx: &wire.MsgTx{
				Version: wire.DefaultBlockVersion,
				TxIn: []*wire.TxIn{{
					PreviousOutPoint: utxo,
