...
// OutPoint defines a PicFight data type that is used to track previous
// transaction outputs.
...
// NewOutPoint returns a new PicFight transaction outpoint point with the
...
// NewTxIn returns a new PicFight transaction input with the provided
...
// NewTxOut returns a new PicFight transaction output with the provided
...
// MsgTx implements the Message interface and represents a PicFight tx message.
...
func writeTxScriptsToMsgTx(msg *MsgTx, totalScriptSize uint64, serType TxSerializeType) {
	...
	//
	// Closures around writing the TxIn and TxOut scripts are used in PicFight
...
// in that BtcDecode decodes from the PicFight wire protocol as it was sent
...
// encodes the transaction to the PicFight wire protocol in order to be sent
// across the network.  The wire encoding can technically differ depending on
// the protocol version and doesn't even really need to match the format of a
// stored transaction at all.  As of the time this comment was written, the
// encoded transaction is the same in both instances, but there is a distinct
...
// NewMsgTx returns a new PicFight tx message that conforms to the Message
...
// WriteOutPoint encodes op to the PicFight protocol encoding for an OutPoint
// to w.
...
// writeTxInPrefixs encodes ti to the PicFight protocol encoding for a transaction
...
// writeTxWitness encodes ti to the PicFight protocol encoding for a transaction
...
// writeTxOut encodes to into the PicFight protocol encoding for a transaction
// output (TxOut) to w.
