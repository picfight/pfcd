...
// MsgFilterLoad implements the Message interface and represents a picfightcoin
...
func (msg *MsgFilterLoad) PfcDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	if pver < BIP0037Version {
		str := fmt.Sprintf("filterload message invalid for protocol "+
			"version %d", pver)
		return messageError("MsgFilterLoad.PfcDecode", str)
	}
...
func (msg *MsgFilterLoad) PfcEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	if pver < BIP0037Version {
		str := fmt.Sprintf("filterload message invalid for protocol "+
			"version %d", pver)
		return messageError("MsgFilterLoad.PfcEncode", str)
	}
