...
// MsgBlock implements the Message interface and represents a picfightcoin
...
func (msg *MsgBlock) PfcDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	if txCount > maxTxPerBlock {
		...
		return messageError("MsgBlock.PfcDecode", str)
	}

	msg.Transactions = make([]*MsgTx, 0, txCount)
	for i := uint64(0); i < txCount; i++ {
		tx := MsgTx{}
		err := tx.PfcDecode(r, pver, enc)
		if err != nil {
			return err
		}
		msg.Transactions = append(msg.Transactions, &tx)
...
// Version field in the block.  This function differs from PfcDecode in that
// PfcDecode decodes from the picfightcoin wire protocol as it was sent across the
...
func (msg *MsgBlock) Deserialize(r io.Reader) error {
	...
	//
	// Passing an encoding type of WitnessEncoding to PfcEncode for the
...
func (msg *MsgBlock) PfcEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
...
// This function differs from PfcEncode in that PfcEncode encodes the block to
// the picfightcoin wire protocol in order to be sent across the network.  The wire
...
// NewMsgBlock returns a new picfightcoin block message that conforms to the
