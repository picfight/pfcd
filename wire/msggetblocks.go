...
// MsgGetBlocks implements the Message interface and represents a picfightcoin
...
func (msg *MsgGetBlocks) PfcDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	if count > MaxBlockLocatorsPerMsg {
		...
		return messageError("MsgGetBlocks.PfcDecode", str)
	}
...
func (msg *MsgGetBlocks) PfcEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	...
	if count > MaxBlockLocatorsPerMsg {
		str := fmt.Sprintf("too many block locator hashes for message "+
			"[count %v, max %v]", count, MaxBlockLocatorsPerMsg)
		return messageError("MsgGetBlocks.PfcEncode", str)
	}
