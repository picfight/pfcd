...
// TestPfcWalletExtCmds tests all of the pfcwallet extended commands marshal and
...
func TestPfcWalletExtCmds(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name: "dumpwallet",
			newCmd: func() (interface{}, error) {
				return pfcjson.NewCmd("dumpwallet", "filename")
			},
			staticCmd: func() interface{} {
				return pfcjson.NewDumpWalletCmd("filename")
			},
			marshalled: `{"jsonrpc":"1.0","method":"dumpwallet","params":["filename"],"id":1}`,
			unmarshalled: &pfcjson.DumpWalletCmd{
				Filename: "filename",
			},
		},
		{
		...
		},
		{
			...
			newCmd: func() (interface{}, error) {
				return pfcjson.NewCmd("importaddress", "1Address", "acct", false)
			},
			staticCmd: func() interface{} {
				return pfcjson.NewImportAddressCmd("1Address", "acct", pfcjson.Bool(false))
			},
		...
		},
		{
			name: "importwallet",
			newCmd: func() (interface{}, error) {
				return pfcjson.NewCmd("importwallet", "filename")
			},
			staticCmd: func() interface{} {
				return pfcjson.NewImportWalletCmd("filename")
			},
			marshalled: `{"jsonrpc":"1.0","method":"importwallet","params":["filename"],"id":1}`,
			unmarshalled: &pfcjson.ImportWalletCmd{
				Filename: "filename",
			},
		},
		{
