...
func TestMarshalResponse(t *testing.T) {
	...
	for i, test := range tests {
		_, _ = i, test
		marshalled, err := pfcjson.MarshalResponse("1.0", testID, test.result, test.jsonErr)
		if err != nil {
...
func TestMiscErrors(t *testing.T) {
	...
	// not supported.
	_, err := pfcjson.NewRequest("1.0", nil, "test", []interface{}{make(chan int)})
	if err == nil {
	...
	// supported.
	wantErr := pfcjson.Error{Code: pfcjson.ErrInvalidType}
	_, err = pfcjson.MarshalResponse("", make(chan int), nil, nil)
	if jerr, ok := err.(pfcjson.Error); !ok || jerr.Code != wantErr.Code {
	...
	// can't be marshalled.
	_, err = pfcjson.MarshalResponse("1.0", 1, make(chan int), nil)
