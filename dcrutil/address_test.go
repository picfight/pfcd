	...
	tests := []struct {
		...
		// Positive P2PKH tests.
		{
			...
			result: dcrutil.TstAddressPubKeyHash(
				...
				chaincfg.DecredNetParams.PubKeyHashAddrID),
			f: func() (dcrutil.Address, error) {
				pkHash := []byte{
					0x27, 0x89, 0xd5, 0x8c, 0xfa, 0x09, 0x57, 0xd2, 0x06, 0xf0,
					0x25, 0xc2, 0xaf, 0x05, 0x6f, 0xc8, 0xa7, 0x7c, 0xeb, 0xb0}
		...
		// Positive P2SH tests.
		{
			...
			result: dcrutil.TstAddressScriptHash(
				...
				chaincfg.DecredNetParams.ScriptHashAddrID),
			f: func() (dcrutil.Address, error) {
				txscript := []byte{
				...
				return dcrutil.NewAddressScriptHash(txscript, &chaincfg.DecredNetParams)
			},
			net: &chaincfg.DecredNetParams,
		},
		{
			...
			f: func() (dcrutil.Address, error) {
				hash := []byte{
					0xc7, 0xda, 0x50, 0x95, 0x68, 0x34, 0x36, 0xf4, 0x43, 0x5f,
					0xc4, 0xe7, 0x16, 0x3d, 0xca, 0xfd, 0xa1, 0xa2, 0xd0, 0x07}
				return dcrutil.NewAddressScriptHashFromHash(hash, &chaincfg.DecredNetParams)
			},
			net: &chaincfg.DecredNetParams,
		},
		{
		...
		// Positive P2PK tests.
		{
			...
			result: dcrutil.TstAddressPubKey(
				[]byte{
					...
					0x0f, 0xe2, 0xed},
				dcrutil.PKFCompressed, chaincfg.DecredNetParams.PubKeyHashAddrID),
			f: func() (dcrutil.Address, error) {
				serializedPubKey := []byte{
					...
					0x52, 0xe7, 0xfe, 0xf0, 0xea, 0x6d, 0xa1, 0x43, 0xa0, 0x2b,
					0x0f, 0xe2, 0xed}
				return dcrutil.NewAddressSecpPubKey(serializedPubKey, &chaincfg.DecredNetParams)
			},
			net: &chaincfg.DecredNetParams,
		},
		{
		...
		},
		{
			...
			result: dcrutil.TstAddressPubKey(
				...
				dcrutil.PKFUncompressed, chaincfg.DecredNetParams.PubKeyHashAddrID),
			f: func() (dcrutil.Address, error) {
				serializedPubKey := []byte{
					0x04, 0x64, 0xc4, 0x46, 0x53, 0xd6, 0x56, 0x7e, 0xff, 0x57,
					0x53, 0xc5, 0xd2, 0x4a, 0x68, 0x2d, 0xdc, 0x2b, 0x2c, 0xad,
