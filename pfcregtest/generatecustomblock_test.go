...
func TestGenerateAndSubmitBlockWithCustomCoinbaseOutputs(t *testing.T) {
	...
	block, err := pfcharness.GenerateAndSubmitBlockWithCustomCoinbaseOutputs(r.NodeRPCClient().(*rpcclient.Client), &newBlockArgs)
	if err != nil {
	...
	block, err = pfcharness.GenerateAndSubmitBlockWithCustomCoinbaseOutputs(r.NodeRPCClient().(*rpcclient.Client), &newBlockArgs2)
	if err != nil {
		t.Fatalf("unable to generate block: %v", err)
	}
