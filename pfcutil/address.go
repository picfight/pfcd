...
import (
	...
	"github.com/picfight/pfcd/pfcec/edwards"
	"github.com/picfight/pfcd/pfcec/secp256k1"
	"github.com/picfight/pfcd/pfcec/secp256k1/schnorr"
)
...
func encodePKAddress(serializedPK []byte, netID [2]byte, algo pfcec.SignatureType) string {
	pubKeyBytes := []byte{0x00}

	switch algo {
	case pfcec.STEcdsaSecp256k1:
		pubKeyBytes[0] = byte(pfcec.STEcdsaSecp256k1)
	case pfcec.STEd25519:
		pubKeyBytes[0] = byte(pfcec.STEd25519)
	case pfcec.STSchnorrSecp256k1:
		pubKeyBytes[0] = byte(pfcec.STSchnorrSecp256k1)
	}

	// Pubkeys are encoded as [0] = type/ybit, [1:33] = serialized pubkey
	compressed := serializedPK
	if algo == pfcec.STEcdsaSecp256k1 || algo == pfcec.STSchnorrSecp256k1 {
...
type Address interface {
	...
	DSA(*chaincfg.Params) pfcec.SignatureType
...
func NewAddressPubKey(decoded []byte, net *chaincfg.Params) (Address, error) {
	if len(decoded) == 33 {
		...
		switch pfcec.SignatureType(suite) {
		case pfcec.STEcdsaSecp256k1:
...
func DecodeAddress(addr string) (Address, error) {
	...
	case net.PubKeyHashAddrID:
		return NewAddressPubKeyHash(decoded, net, pfcec.STEcdsaSecp256k1)

	case net.PKHEdwardsAddrID:
		return NewAddressPubKeyHash(decoded, net, pfcec.STEd25519)

	case net.PKHSchnorrAddrID:
		return NewAddressPubKeyHash(decoded, net, pfcec.STSchnorrSecp256k1)
...
// hash.
func (a *AddressPubKeyHash) DSA(net *chaincfg.Params) pfcec.SignatureType {
	switch a.netID {
	case net.PubKeyHashAddrID:
		return pfcec.STEcdsaSecp256k1
	case net.PKHEdwardsAddrID:
		return pfcec.STEd25519
	case net.PKHSchnorrAddrID:
		return pfcec.STSchnorrSecp256k1
	}
	return -1
}
...
func (a *AddressScriptHash) DSA(net *chaincfg.Params) pfcec.SignatureType {
	return -1
}
...
func NewAddressSecpPubKey(serializedPubKey []byte,
	...
	// from pfcec, but do it here to avoid API churn.  We already know the
...
// address.
func (a *AddressSecpPubKey) DSA(net *chaincfg.Params) pfcec.SignatureType {
...
// DSA returns the underlying digital signature algorithm for the
// address.
func (a *AddressEdwardsPubKey) DSA(net *chaincfg.Params) pfcec.SignatureType {
	return pfcec.STEd25519
}
...
// DSA returns the underlying digital signature algorithm for the
// address.
func (a *AddressSecSchnorrPubKey) DSA(net *chaincfg.Params) pfcec.SignatureType {
	return pfcec.STSchnorrSecp256k1
}
