...
// Tx returns a wrapped transaction (pfcutil.Tx) for the transaction at the
...
// STx returns a wrapped transaction (pfcutil.Tx) for the stake transaction at
...
// Transactions returns a slice of wrapped transactions (pfcutil.Tx) for all
...
// instead provides easy access to wrapped versions (pfcutil.Tx) of them.
...
// STransactions returns a slice of wrapped stake transactions (pfcutil.Tx) for all
// stake transactions in the Block.  This is nearly equivalent to accessing the raw
// transactions (pfcwire.MsgTx) in the underlying wire.MsgBlock, however it
