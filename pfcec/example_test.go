...
func Example_signMessage() {
	...
	privKey, pubKey := pfcec.PrivKeyFromBytes(pfcec.S256(), pkBytes)
...
func Example_verifySignature() {
	...
	pubKey, err := pfcec.ParsePubKey(pubKeyBytes, pfcec.S256())
	if err != nil {
		fmt.Println(err)
		return
	}
...
func Example_encryptMessage() {
	...
	message := "test message"
	ciphertext, err := pfcec.Encrypt(pubKey, []byte(message))
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	privKey, _ := pfcec.PrivKeyFromBytes(pfcec.S256(), pkBytes)

	// Try decrypting and verify if it's the same message.
	plaintext, err := pfcec.Decrypt(privKey, ciphertext)
	if err != nil {
		fmt.Println(err)
		return
	}
