...
type Config struct {
	...
	FetchUtxoView func(*pfcutil.Tx, bool) (*blockchain.UtxoViewpoint, error)

	// BlockByHash defines the function use to fetch the block identified
	// by the given hash.
	BlockByHash func(*chainhash.Hash) (*pfcutil.Block, error)
...
type TxPool struct {
	...
	orphans       map[chainhash.Hash]*pfcutil.Tx
...
func (mp *TxPool) insertVote(ssgen *pfcutil.Tx) error {
	msgTx := ssgen.MsgTx()
	...
	vote := pfcutil.IsFlagSet16(voteBits, pfcutil.BlockValid)
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addOrphan(tx *pfcutil.Tx) {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAddOrphan(tx *pfcutil.Tx) error {
...
func (mp *TxPool) addTransaction(utxoView *blockchain.UtxoViewpoint,
	tx *pfcutil.Tx, txType stake.TxType, height int64, fee int64) {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) checkPoolDoubleSpend(tx *pfcutil.Tx, txType stake.TxType) error {
...
// It should also set the pfcutil tree type for the tx as well.
func (mp *TxPool) maybeAcceptTransaction(tx *pfcutil.Tx, isNew, rateLimit, allowHighFees, rejectDupOrphans bool) ([]*chainhash.Hash, error) {
	msgTx := tx.MsgTx()
	txHash := tx.Hash()
...
//
// This function is safe for concurrent access.
func (mp *TxPool) MaybeAcceptTransaction(tx *pfcutil.Tx, isNew, rateLimit bool) ([]*chainhash.Hash, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) processOrphans(acceptedTx *pfcutil.Tx) []*pfcutil.Tx {
	var acceptedTxns []*pfcutil.Tx
...
//
// This function is safe for concurrent access.
func (mp *TxPool) ProcessTransaction(tx *pfcutil.Tx, allowOrphan, rateLimit, allowHighFees bool) ([]*pfcutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
	defer mp.mtx.Unlock()
	var err error
	...
	if len(missingParents) == 0 {
		...
		acceptedTxs := make([]*pfcutil.Tx, len(newTxs)+1)
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RawMempoolVerbose(filterType *stake.TxType) map[string]*pfcjson.GetRawMempoolVerboseResult {
	mp.mtx.RLock()
	...
	for _, desc := range mp.pool {
		...
		mpd := &pfcjson.GetRawMempoolVerboseResult{
			...
			Fee:              pfcutil.Amount(desc.Fee).ToCoin(),
...
func New(cfg *Config) *TxPool {
	return &TxPool{
		...
		orphans:       make(map[chainhash.Hash]*pfcutil.Tx),
