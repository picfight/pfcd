...
type TxPool struct {
	...
	orphansByPrev map[chainhash.Hash]map[chainhash.Hash]*pfcutil.Tx
...
func (mp *TxPool) maybeAcceptTransaction(tx *pfcutil.Tx, isNew, rateLimit, allowHighFees bool) ([]*chainhash.Hash, error) {
	msgTx := tx.MsgTx()
	txHash := tx.Hash()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) processOrphans(hash *chainhash.Hash) []*pfcutil.Tx {
...
func New(cfg *Config) *TxPool {
	return &TxPool{
		...
		orphansByPrev: make(map[chainhash.Hash]map[chainhash.Hash]*pfcutil.Tx),
