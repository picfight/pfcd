...
import (
	...
	peerpkg "github.com/picfight/pfcd/peer"
...
// blockMsg packages a picfightcoin block message and the peer it came from together
...
// invMsg packages a picfightcoin inv message and the peer it came from together
// so the block handler has access to that information.
type invMsg struct {
	inv  *wire.MsgInv
	peer *peerpkg.Peer
...
// headersMsg packages a picfightcoin headers message and the peer it came from
...
// txMsg packages a picfightcoin tx message and the peer it came from together
// so the block handler has access to that information.
type txMsg struct {
	tx    *pfcutil.Tx
	peer  *peerpkg.Peer
...
func (sm *SyncManager) handleTxMsg(tmsg *txMsg) {
	...
	// NOTE:  PicfightcoinJ, and possibly other wallets, don't follow the spec of
...
// processed.
func (sm *SyncManager) QueueTx(tx *pfcutil.Tx, peer *peerpkg.Peer, done chan struct{}) {
...
// processed.
func (sm *SyncManager) QueueBlock(block *pfcutil.Block, peer *peerpkg.Peer, done chan struct{}) {
...
// chain.
func (sm *SyncManager) ProcessBlock(block *pfcutil.Block, flags blockchain.BehaviorFlags) (bool, error) {
